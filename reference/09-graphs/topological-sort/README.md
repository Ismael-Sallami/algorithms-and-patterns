...
A linear order in which every edge points forwards. It exists exactly when the graph is acyclic, so the algorithm doubles as a cycle test - and that is usually the more useful half.
...
```
g++ -std=c++20 -O1 -Wall -o /tmp/check topological_sort.cpp && /tmp/check
```
